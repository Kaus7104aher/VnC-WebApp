...
export async function getRecommendedUsers(req,res){
   try {
      ...
      console.log("Error in getReccommendedUsers controller", error.message);
...
export async function sendFriendRequest(req,res){
   try {
      ...
      //prevent req from sending to ourself
      if(myId===recipientId){
         res.status(400).json({message: "You can Send req to YourSelf"});
      }
      ...
      if(existingRequest){
         return res
         .status(400)
         .json({message: "A friend request exists between you and this user"})
      }
